...
export function useMapInteraction() {
  ...
  const [lastQueriedBounds, setLastQueriedBounds] = useState<MapBounds | null>(null)
  const [ghostPosition, setGhostPosition] = useState<[number, number] | null>(null)
  const [showSearchButton, setShowSearchButton] = useState(false)

  // Calculate if map has moved significantly
  const hasMovedSignificantly = useCallback((current: MapBounds, last: MapBounds | null): boolean => {
    if (!last) return true
    ...
    const viewportHeight = current.bounds.getNorth() - current.bounds.getSouth()
  ...
  const handleMapChange = useCallback(
    (bounds: MapBounds) => {
      ...
      if (bounds.zoom >= 5 && hasMovedSignificantly(bounds, lastQueriedBounds)) {
