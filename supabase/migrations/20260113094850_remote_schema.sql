...
CREATE TYPE "public"."CarLevel" AS ENUM (
    'street',
    'semi_pro',
    'pro'
);
...
ALTER TYPE "public"."CarLevel" OWNER TO "postgres";
...
COMMENT ON TYPE "public"."RegistrationStatus" IS 'active, cancelled (cash only), paid - online';
...
CREATE OR REPLACE FUNCTION "public"."check_free_places_before_insert"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  free int;
BEGIN
  SELECT max_drivers - registered_drivers
  INTO free
  FROM events
  WHERE id = NEW.event_id;

  IF free <= 0 THEN
    RAISE EXCEPTION 'No free places left for this event';
  END IF;
...
ALTER FUNCTION "public"."check_free_places_before_insert"() OWNER TO "postgres";
...
DECLARE
  ...
  location_name TEXT;
BEGIN
  -- Получаем название локации (если location_id не задан, будет NULL)
  SELECT name INTO location_name
  FROM locations
  WHERE id = NEW.location_id;

  -- Формируем базовый slug из level, type, title, location_name
  base_slug := lower(
    regexp_replace(
      array_to_string(
        array_remove(ARRAY[NEW.level::text,
    NEW.type::text, NEW.title, location_name], NULL),
        '-'
      ),
      '[^a-zA-Z0-9-]', '', 'g'
    )
  ...
  -- Если base_slug пустой, ставим fallback
  ...
  -- Пытаемся вставить уникальный slug, добавляя короткий UUID-суффикс при конфликте
...
CREATE OR REPLACE FUNCTION "public"."get_events_grouped_by_location"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[] DEFAULT NULL::"public"."EventType"[], "p_level" "text"[] DEFAULT NULL::"text"[], "price_min" numeric DEFAULT NULL::numeric, "price_max" numeric DEFAULT NULL::numeric, "date_from" timestamp with time zone DEFAULT NULL::timestamp with time zone, "date_to" timestamp with time zone DEFAULT NULL::timestamp with time zone) RETURNS "jsonb"
  ...
  cross join lateral (
    select jsonb_agg(
      jsonb_build_object(
        ...
        'level', e.level,
        'type', e.type,
        'registered_drivers', e.registered_drivers,
        'slug', e.slug,
    ...
    where e.location_id = l.id
      and (p_level is null or e.level = any(p_level))
      and (p_type is null or e.type = any(p_type))
      and (price_min is null or e.price >= price_min)
      and (price_max is null or e.price <= price_max)
      and (
  (e.start_date, e.end_date) overlaps (
    coalesce(date_from, '-infinity'::timestamptz),
    coalesce(date_to, 'infinity'::timestamptz)
  )
)
  ) ev
  where
    ...
    -- вернуть только локации с событиями (как было при inner join)
    and ev.events is not null
  ;
...
ALTER FUNCTION "public"."get_events_grouped_by_location"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[], "p_level" "text"[], "price_min" numeric, "price_max" numeric, "date_from" timestamp with time zone, "date_to" timestamp with time zone) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_events_grouped_by_location"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[] DEFAULT NULL::"public"."EventType"[], "p_level" "text"[] DEFAULT NULL::"text"[], "price_min" numeric DEFAULT NULL::numeric, "price_max" numeric DEFAULT NULL::numeric, "date_from" timestamp with time zone DEFAULT NULL::timestamp with time zone, "date_to" timestamp with time zone DEFAULT NULL::timestamp with time zone, "free_min" integer DEFAULT NULL::integer) RETURNS "jsonb"
    LANGUAGE "plpgsql" STABLE
    AS $$
declare
  result jsonb;
begin
  ...
  cross join lateral (
    ...
    where e.location_id = l.id
      ...
      and (
        free_min is null
        or (e.max_drivers - e.registered_drivers) >= free_min
      )
      and (price_min is null or e.price >= price_min)
      and (price_max is null or e.price <= price_max)
      and (
...
ALTER FUNCTION "public"."get_events_grouped_by_location"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[], "p_level" "text"[], "price_min" numeric, "price_max" numeric, "date_from" timestamp with time zone, "date_to" timestamp with time zone, "free_min" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_locations_with_event_ids"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[] DEFAULT NULL::"public"."EventType"[], "p_level" "text"[] DEFAULT NULL::"text"[], "price_min" numeric DEFAULT NULL::numeric, "price_max" numeric DEFAULT NULL::numeric, "date_from" timestamp with time zone DEFAULT NULL::timestamp with time zone, "date_to" timestamp with time zone DEFAULT NULL::timestamp with time zone, "free_min" integer DEFAULT NULL::integer) RETURNS TABLE("location_id" "uuid", "name" "text", "address" "text", "geom" "public"."geography", "event_count" bigint, "event_ids" "uuid"[])
    LANGUAGE "sql" STABLE
    AS $$
  select 
    l.id,
  ...
  where e.geom && ST_MakeEnvelope(west, south, east, north, 4326)
    and e.deleted_at is null
    and e.status = 'active'
    and e.end_date >= now() at time zone 'UTC'
    and (p_type    is null or e.type    = any(p_type))
    and (p_level   is null or e.level   = any(p_level))
    and (price_min is null or e.price  >= price_min)
    and (price_max is null or e.price  <= price_max)
    and (free_min  is null or e.free_places >= free_min)
    and (date_from is null or e.end_date   >= date_from)
    and (date_to   is null or e.start_date <= date_to)
  group by l.id, l.name, l.address, l.geom;
$$;


ALTER FUNCTION "public"."get_locations_with_event_ids"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[], "p_level" "text"[], "price_min" numeric, "price_max" numeric, "date_from" timestamp with time zone, "date_to" timestamp with time zone, "free_min" integer) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."locations_in_bbox"("west" double precision, "south" double precision, "east" double precision, "north" double precision) RETURNS TABLE("id" "uuid", "name" "text", "address" "text", "geom" "public"."geography")
    LANGUAGE "sql" STABLE
    AS $$
  select 
    l.id,
    ...
    l.geom
  from locations l
  where l.geom && ST_MakeEnvelope(west, south, east, north, 4326);
$$;


ALTER FUNCTION "public"."locations_in_bbox"("west" double precision, "south" double precision, "east" double precision, "north" double precision) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."soft_delete_car"("p_car_id" "uuid") RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
begin
  -- 1. Проверка: есть ли активные регистрации на будущие активные события
  if exists (
    select 1
    ...
    raise exception 'CAR_IN_USE'
      using errcode = 'P0001';
  end if;

  -- 2. Soft delete + возвращаем true только если строка действительно обновилась
  update cars
  set deleted_at = now() at time zone 'utc'
  where id = p_car_id
    and deleted_at is null;

  -- found — встроенная переменная PL/pgSQL, true если UPDATE затронул ≥1 строку
  return found;
end;
$$;


ALTER FUNCTION "public"."soft_delete_car"("p_car_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."soft_delete_event"("p_event_id" "uuid", "p_user_id" "uuid") RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    -- Пытаемся обновить событие, если владелец + статус active/pending + нет участников
    UPDATE events
    ...
    WHERE id = p_event_id
      ...
      AND status IN ('active'::"Status", 'pending'::"Status")
      AND registered_drivers = 0;

    -- FOUND будет true, если хотя бы одна строка обновилась
    RETURN FOUND;
END;
$$;


ALTER FUNCTION "public"."soft_delete_event"("p_event_id" "uuid", "p_user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."soft_delete_registration"("p_registration_id" "uuid", "p_user_id" "uuid") RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    UPDATE registrations
    SET status = 'cancelled'::"RegistrationStatus",
        deleted_at = now()
    WHERE id = p_registration_id
      AND user_id = p_user_id
      AND status IN ('pending'::"RegistrationStatus", 'active'::"RegistrationStatus");
...
ALTER FUNCTION "public"."soft_delete_registration"("p_registration_id" "uuid", "p_user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."sync_event_geom_safe"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
begin
  -- Обновляем geom у текущей строки (или всех, если меняли location)
  update events e
  set geom = l.geom
  from locations l
  where l.id = e.location_id
    and e.id = NEW.id
    and (e.geom is distinct from l.geom or e.geom is null);

  return null; -- AFTER trigger → return игнорируется
end;
$$;


ALTER FUNCTION "public"."sync_event_geom_safe"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."sync_events_after_location_move"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
begin
  if (OLD.geom is distinct from NEW.geom) then
    ...
    where location_id = NEW.id
      and deleted_at is null
      and (geom is distinct from NEW.geom);
  end if;
  return NEW;
end;
$$;
...
ALTER FUNCTION "public"."sync_events_after_location_move"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."update_event_free_places"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  -- Вычисляем свободные места
  NEW.free_places := GREATEST(NEW.max_drivers - NEW.registered_drivers, 0);
...
ALTER FUNCTION "public"."update_event_free_places"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."update_registered_drivers"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  UPDATE events
  SET registered_drivers = (
...
ALTER FUNCTION "public"."update_registered_drivers"() OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."cars" (
    ...
    "level" "public"."CarLevel" DEFAULT 'street'::"public"."CarLevel" NOT NULL
);
...
CREATE TABLE IF NOT EXISTS "public"."events" (
    ...
    "deleted_at" timestamp with time zone,
    "registered_drivers" smallint DEFAULT 0 NOT NULL,
    "slug" "text" NOT NULL,
    "location_id" "uuid" NOT NULL,
    "image_url" "text",
    "type" "public"."EventType" DEFAULT 'training'::"public"."EventType" NOT NULL,
    "end_date" timestamp with time zone NOT NULL,
    "free_places" smallint DEFAULT '0'::smallint NOT NULL,
    "geom" "public"."geography"(Point,4326),
    "description_short" "text" GENERATED ALWAYS AS (("left"(COALESCE("description", ''::"text"), 160) ||
CASE
    WHEN ("length"(COALESCE("description", ''::"text")) > 160) THEN '…'::"text"
    ELSE ''::"text"
END)) STORED,
    "payment_type" "public"."PaymentType" DEFAULT 'cash'::"public"."PaymentType" NOT NULL,
    CONSTRAINT "events_end_after_start" CHECK ((("start_date" > "now"()) AND ("end_date" > "start_date") AND ("end_date" > "now"()))),
    CONSTRAINT "events_end_date_check" CHECK (("end_date" > "now"())),
    CONSTRAINT "events_level_check" CHECK (("level" = ANY (ARRAY['beginner'::"text", 'street'::"text", 'pro'::"text"]))),
    CONSTRAINT "events_max_drivers_check" CHECK (("max_drivers" > 0)),
    CONSTRAINT "events_price_check" CHECK (("price" >= (0)::numeric)),
    CONSTRAINT "events_start_date_check" CHECK (("start_date" > "now"())),
    CONSTRAINT "registered_drivers_check" CHECK (("registered_drivers" <= "max_drivers"))
);
...
CREATE TABLE IF NOT EXISTS "public"."registration_codes" (
    ...
    CONSTRAINT "registration_codes_code_check" CHECK ((("code" >= 100000) AND ("code" <= 999999)))
);
...
CREATE TABLE IF NOT EXISTS "public"."registrations" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"(),
    "deleted_at" timestamp with time zone,
    "status" "public"."RegistrationStatus" DEFAULT 'active'::"public"."RegistrationStatus" NOT NULL,
    "payment_type" "public"."PaymentType" DEFAULT 'cash'::"public"."PaymentType" NOT NULL,
...
COMMENT ON COLUMN "public"."registrations"."deleted_at" IS 'when registration was cancelled (e.g user cancel, auto-cancel)';
...
CREATE OR REPLACE VIEW "public"."view_registered_counts" AS
 SELECT "event_id",
    "count"(*) AS "registered_drivers"
   FROM "public"."registrations"
  GROUP BY "event_id";


ALTER VIEW "public"."view_registered_counts" OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."registrations"
    ADD CONSTRAINT "registrations_payment_intent_id_key" UNIQUE ("payment_intent_id");
...
CREATE INDEX "idx_events_free_places" ON "public"."events" USING "btree" ("free_places");



CREATE INDEX "idx_events_geom" ON "public"."events" USING "gist" ("geom") WHERE ("deleted_at" IS NULL);
...
CREATE INDEX "idx_events_price" ON "public"."events" USING "btree" ("price");
...
CREATE INDEX "idx_registration_codes_code" ON "public"."registration_codes" USING "btree" ("code");



CREATE INDEX "idx_registration_codes_expires" ON "public"."registration_codes" USING "btree" ("expires_at");
...
CREATE UNIQUE INDEX "registrations_active_unique" ON "public"."registrations" USING "btree" ("user_id", "event_id") WHERE ("status" = ANY (ARRAY['pending'::"public"."RegistrationStatus", 'active'::"public"."RegistrationStatus"]));



CREATE OR REPLACE TRIGGER "events_slug_unique_trigger" BEFORE INSERT ON "public"."events" FOR EACH ROW EXECUTE FUNCTION "public"."generate_unique_slug"();



CREATE OR REPLACE TRIGGER "trg_registrations_before_insert" BEFORE INSERT ON "public"."registrations" FOR EACH ROW EXECUTE FUNCTION "public"."check_free_places_before_insert"();



CREATE OR REPLACE TRIGGER "trg_registrations_insert" AFTER INSERT ON "public"."registrations" FOR EACH ROW EXECUTE FUNCTION "public"."update_registered_drivers"();



CREATE OR REPLACE TRIGGER "trg_registrations_update_deleted_at" AFTER UPDATE OF "deleted_at" ON "public"."registrations" FOR EACH ROW WHEN (("old"."deleted_at" IS DISTINCT FROM "new"."deleted_at")) EXECUTE FUNCTION "public"."update_registered_drivers"();



CREATE OR REPLACE TRIGGER "trg_sync_event_geom" AFTER INSERT OR UPDATE OF "location_id" ON "public"."events" FOR EACH ROW EXECUTE FUNCTION "public"."sync_event_geom_safe"();



CREATE OR REPLACE TRIGGER "trg_sync_events_on_location_move" AFTER UPDATE OF "geom" ON "public"."locations" FOR EACH ROW WHEN (("old"."geom" IS DISTINCT FROM "new"."geom")) EXECUTE FUNCTION "public"."sync_events_after_location_move"();



CREATE OR REPLACE TRIGGER "trg_update_event_free_places" BEFORE INSERT OR UPDATE OF "registered_drivers", "max_drivers" ON "public"."events" FOR EACH ROW EXECUTE FUNCTION "public"."update_event_free_places"();
...
ALTER TABLE ONLY "public"."registration_codes"
    ADD CONSTRAINT "registration_codes_registration_id_fkey" FOREIGN KEY ("registration_id") REFERENCES "public"."registrations"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."registrations"
    ADD CONSTRAINT "registrations_car_id_fkey" FOREIGN KEY ("car_id") REFERENCES "public"."cars"("id") ON DELETE CASCADE;
...
CREATE POLICY "Anyone can view non-cancelled registrations" ON "public"."registrations" FOR SELECT USING (("status" <> 'cancelled'::"public"."RegistrationStatus"));



CREATE POLICY "Anyone can view non-deleted events" ON "public"."events" FOR SELECT USING (("status" <> 'deleted'::"public"."Status"));
...
CREATE POLICY "Everyone can view non-deleted cars" ON "public"."cars" FOR SELECT USING (("deleted_at" IS NULL));



CREATE POLICY "Users can add thei own cars" ON "public"."cars" FOR INSERT WITH CHECK ((("auth"."uid"() = "user_id") AND ("deleted_at" IS NULL)));
...
CREATE POLICY "Users can update their own cars" ON "public"."cars" FOR UPDATE USING ((("auth"."uid"() = "user_id") AND ("deleted_at" IS NULL))) WITH CHECK ((("auth"."uid"() = "user_id") AND ("deleted_at" IS NULL)));



CREATE POLICY "Users can update their own events" ON "public"."events" FOR UPDATE USING (("auth"."uid"() = "created_by")) WITH CHECK (("auth"."uid"() = "created_by"));



CREATE POLICY "Users can update their registrations" ON "public"."registrations" FOR UPDATE USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));
...
CREATE POLICY "Users can't delete events" ON "public"."events" FOR DELETE USING (false);



CREATE POLICY "Users can't delete registrations" ON "public"."registrations" FOR DELETE USING (false);



CREATE POLICY "Users can't hard-delete cars" ON "public"."cars" FOR DELETE USING (false);
...
GRANT ALL ON FUNCTION "public"."check_free_places_before_insert"() TO "anon";
GRANT ALL ON FUNCTION "public"."check_free_places_before_insert"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."check_free_places_before_insert"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_events_grouped_by_location"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[], "p_level" "text"[], "price_min" numeric, "price_max" numeric, "date_from" timestamp with time zone, "date_to" timestamp with time zone) TO "anon";
GRANT ALL ON FUNCTION "public"."get_events_grouped_by_location"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[], "p_level" "text"[], "price_min" numeric, "price_max" numeric, "date_from" timestamp with time zone, "date_to" timestamp with time zone) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_events_grouped_by_location"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[], "p_level" "text"[], "price_min" numeric, "price_max" numeric, "date_from" timestamp with time zone, "date_to" timestamp with time zone) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_events_grouped_by_location"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[], "p_level" "text"[], "price_min" numeric, "price_max" numeric, "date_from" timestamp with time zone, "date_to" timestamp with time zone, "free_min" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_events_grouped_by_location"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[], "p_level" "text"[], "price_min" numeric, "price_max" numeric, "date_from" timestamp with time zone, "date_to" timestamp with time zone, "free_min" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_events_grouped_by_location"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[], "p_level" "text"[], "price_min" numeric, "price_max" numeric, "date_from" timestamp with time zone, "date_to" timestamp with time zone, "free_min" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_locations_with_event_ids"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[], "p_level" "text"[], "price_min" numeric, "price_max" numeric, "date_from" timestamp with time zone, "date_to" timestamp with time zone, "free_min" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_locations_with_event_ids"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[], "p_level" "text"[], "price_min" numeric, "price_max" numeric, "date_from" timestamp with time zone, "date_to" timestamp with time zone, "free_min" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_locations_with_event_ids"("north" double precision, "south" double precision, "east" double precision, "west" double precision, "p_type" "public"."EventType"[], "p_level" "text"[], "price_min" numeric, "price_max" numeric, "date_from" timestamp with time zone, "date_to" timestamp with time zone, "free_min" integer) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."locations_in_bbox"("west" double precision, "south" double precision, "east" double precision, "north" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."locations_in_bbox"("west" double precision, "south" double precision, "east" double precision, "north" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."locations_in_bbox"("west" double precision, "south" double precision, "east" double precision, "north" double precision) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."soft_delete_car"("p_car_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."soft_delete_car"("p_car_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."soft_delete_car"("p_car_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."soft_delete_event"("p_event_id" "uuid", "p_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."soft_delete_event"("p_event_id" "uuid", "p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."soft_delete_event"("p_event_id" "uuid", "p_user_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."soft_delete_registration"("p_registration_id" "uuid", "p_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."soft_delete_registration"("p_registration_id" "uuid", "p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."soft_delete_registration"("p_registration_id" "uuid", "p_user_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."sync_event_geom_safe"() TO "anon";
GRANT ALL ON FUNCTION "public"."sync_event_geom_safe"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."sync_event_geom_safe"() TO "service_role";



GRANT ALL ON FUNCTION "public"."sync_events_after_location_move"() TO "anon";
GRANT ALL ON FUNCTION "public"."sync_events_after_location_move"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."sync_events_after_location_move"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_event_free_places"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_event_free_places"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_event_free_places"() TO "service_role";



GRANT ALL ON FUNCTION "public"."update_registered_drivers"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_registered_drivers"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_registered_drivers"() TO "service_role";
...
GRANT ALL ON TABLE "public"."registration_codes" TO "anon";
GRANT ALL ON TABLE "public"."registration_codes" TO "authenticated";
GRANT ALL ON TABLE "public"."registration_codes" TO "service_role";
...
GRANT ALL ON TABLE "public"."view_registered_counts" TO "anon";
GRANT ALL ON TABLE "public"."view_registered_counts" TO "authenticated";
GRANT ALL ON TABLE "public"."view_registered_counts" TO "service_role";
...
drop trigger if exists "trg_sync_events_on_location_move" on "public"."locations";

CREATE TRIGGER trg_sync_events_on_location_move AFTER UPDATE OF geom ON public.locations FOR EACH ROW WHEN ((old.geom IS DISTINCT FROM new.geom)) EXECUTE FUNCTION public.sync_events_after_location_move();
